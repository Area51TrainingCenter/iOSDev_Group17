...
class RegistroTableViewController: UITableViewController {
    ...
    var fotoSeleccionada : UIImage? = nil
                    ...
                    if txtOcupacion.text != nil && txtOcupacion.text != "" {
                        if txtFechaNacimiento.text != nil && txtFechaNacimiento.text != "" {
                            ...
                            if fotoSeleccionada != nil {
                                self.cliente?.foto = fotoSeleccionada
                            }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if fila == 0 {
            ...
            let alertController = UIAlertController(title: "Imagen", message: "De donde quieres sacar la imagen", preferredStyle: .actionSheet)
            
            let cameraAction = UIAlertAction(title: "Camara", style: .default, handler: { (action) in
                self.picker.sourceType = .camera
                self.picker.delegate = self
                self.present(self.picker, animated: true, completion: nil)
            })
            let galleryAction = UIAlertAction(title: "Galeria", style: .default, handler: { (action) in
...
extension RegistroTableViewController : UINavigationControllerDelegate {}

extension RegistroTableViewController : UIImagePickerControllerDelegate {

    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        
        if let imagenSeleccionada = info[UIImagePickerControllerOriginalImage] as? UIImage {
            self.fotoSeleccionada = imagenSeleccionada
            self.imgFoto.image = self.fotoSeleccionada
        }
        else{
